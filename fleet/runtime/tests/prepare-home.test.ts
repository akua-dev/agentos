...
const mateRuntime = join(repository, "fleet", "runtime");
...
`,
    ...
    const environment = {
      ...
      MISE_SYSTEM_CONFIG_FILE: join(repository, "agentos", "mise.toml"),
    ...
    ).toBe(await readFile(join(repository, "fleet", "mise.toml"), "utf8"));
    expect(
      await readFile(join(home, ".config", "mise", "mise.lock"), "utf8"),
    ).toBe(await readFile(join(repository, "fleet", "mise.lock"), "utf8"));
    expect(
      await readFile(
    ...
    expect((await readFile(join(logDirectory, "mise.log"), "utf8")).trim().split("\n")).toEqual([
      `trust ${join(repository, "agentos", "mise.toml")}`,
