...
`,
  ...
  const env = {
    ...
    AGENTOS_AGENT_CWD: join(repository, "fleet", "agents", "firstmate"),
...
describe("Mate runtime", () => {
  test("starts one named Pi agent on an empty Herdr session", async () => {
    ...
    const expectedStart = [
      ...
      "firstmate",
      "--cwd",
      join(repository, "fleet", "agents", "firstmate"),
      "--no-focus",
      "--session",
  ...
  test("runs and checks the configured Second Mate identity", async () => {
    const secondMateCwd = join(repository, "fleet", "agents", "secondmate");
    const { env, state } = await createHarness([], {
