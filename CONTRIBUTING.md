...
`1.4.0-canary.1+1ff145449`. Bun publishes that build under a moving `canary`
release, so the reviewed `mise.lock` deliberately uses each GitHub release
asset's immutable API ID as both download URLs. Never commit the mutable
`releases/download/canary/...` URLs generated by a plain `mise lock`.

To review a Bun upgrade, refresh only that tool with
`mise lock github:oven-sh/bun`, replace every generated Bun platform `url`
with its paired `url_api`, then verify the upstream checksums, a cold
`mise install --locked github:oven-sh/bun` on every released platform,
`bun --revision`, `bun run check`, and an exact-commit image build. Update the
Dockerfile revision assertion in the same change. A stable `bun-v1.4.0` or
newer release should replace this canary pin when available.
