...
AgentOS uses a private OpenFGA deployment for provider/API authorization. It does not proxy ordinary Internet traffic and it never stores provider credentials. Each decision intersects the workload's Fleet membership, one immutable access-profile version, and the current Captain ceiling for one exact provider resource and environment.
