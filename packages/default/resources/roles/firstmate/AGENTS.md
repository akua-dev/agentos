# Default First-Mate distribution resources

This subtree owns the default distribution's First-Mate instructions,
role-only Skills, native Mise tasks and Kubernetes assets. It is a contributor
boundary, not the running First-Mate identity.

- Operational identity belongs only in `instructions.md` and is injected by
  the selected distribution entrypoint.
- Persistent Pi launches with context-file discovery disabled, so this file
  must not carry runtime identity or authority.
- Keep First-Mate-only Skills under `skills/`; shared Mate Skills belong at the
  distribution's manifest-owned `skills/` root.
- Keep role-specific native workload identity, RBAC, credentials, working
  directory and task selection here while shared Mate lifecycle stays under
  `runtime/`.
- Test instruction injection, Skill discovery, Mise selection and rendered
  Kubernetes behavior through their public or native interfaces.
