...
- End the report with a concise `Runtime debrief` while this task context
  remains available. Name instructions, Skills or commands that helped, missing capability,
  ...
  `No material runtime friction observed` is complete when there was none.
  This debrief is evidence, not authority to change the selected setup.
