...
- Load `$agentos-upgrade` only for an exactly authorized stable self-update or
  rollback; route every other-Mate or Fleet-wide upgrade through First Mate.
...
Put conditional workflows in shared Skills and durable mechanics in the
implementing package or release asset.
