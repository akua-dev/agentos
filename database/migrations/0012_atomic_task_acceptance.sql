...
BEGIN
  ...
  IF NOT agentos.valid_composition_manifest(p_dispatch_profile) THEN
    RAISE EXCEPTION
      'Task acceptance requires a valid concrete dispatch profile';
  END IF;
  ...
  IF p_dispatch_profile ->> 'harness'
       IS DISTINCT FROM v_agent.harness THEN
  ...
  v_request := jsonb_build_object(
    ...
    'assignment', jsonb_build_object(
      ...
      'brief', p_brief,
      'dispatch_profile', p_dispatch_profile,
...
BEGIN
  ...
  EXECUTE format(
    'REVOKE EXECUTE ON FUNCTION agentos.retire_agent(uuid, text), agentos.provision_agent(text, text, text, text, text, jsonb), agentos.handoff_task_assignment(uuid, uuid, text, text, text, jsonb), agentos.hold_captain_decision(uuid, text, text, text, text), agentos.link_task_decision(uuid, text, text), agentos.attest_assignment_decisions(uuid, text[]), agentos.resolve_captain_decision(uuid, text, text), agentos.create_task_with_assignment(uuid, uuid, uuid, uuid, uuid, text, text, text, text, text, jsonb, jsonb, jsonb, text, text, text, text, jsonb, jsonb), agentos.accept_backlog_task(uuid, uuid, uuid, text, text, text, text, text, text, jsonb, jsonb) FROM %I',
    p_database_role
  );
  EXECUTE format(
  ...
  IF p_agent_role = 'second_mate' THEN
    ...
    EXECUTE format(
      'GRANT EXECUTE ON FUNCTION agentos.retire_agent(uuid, text), agentos.provision_agent(text, text, text, text, text, jsonb), agentos.handoff_task_assignment(uuid, uuid, text, text, text, jsonb), agentos.hold_captain_decision(uuid, text, text, text, text), agentos.link_task_decision(uuid, text, text), agentos.attest_assignment_decisions(uuid, text[]), agentos.resolve_captain_decision(uuid, text, text), agentos.create_task_with_assignment(uuid, uuid, uuid, uuid, uuid, text, text, text, text, text, jsonb, jsonb, jsonb, text, text, text, text, jsonb, jsonb), agentos.accept_backlog_task(uuid, uuid, uuid, text, text, text, text, text, text, jsonb, jsonb) TO %I',
      p_database_role
    );
    EXECUTE format(
