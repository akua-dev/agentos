...
BEGIN
  ...
  IF v_active_rows IS NOT NULL THEN
    RAISE EXCEPTION
      'Migration 0014 cannot remove active Captain intent. First preserve active Captain rows in the owning Mate''s $HOME/memory/, verify dynamic injection, archive the preserved or superseded rows, and retry. Active rows: %',
      v_active_rows;
  END IF;
END;
$$;
