...
BEGIN
  ...
  IF v_active_rows IS NOT NULL THEN
    RAISE EXCEPTION
      'Migration 0015 cannot remove active Captain intent. First preserve active Captain rows in the owning Mate''s $HOME/MEMORY.md, verify dynamic injection, archive the preserved or superseded rows, and retry. Active rows: %',
      v_active_rows;
  END IF;
END;
$$;
