...
2. Load `$agentos-composition`, then select the smallest reviewed composition
   that fits the Task, explicit Captain choices and applicable
   non-authoritative working preferences. Resolve the concrete harness,
   ...
   Load `$agentos-harnesses`; consult the current Mate's dynamically loaded
   `$HOME/MEMORY.md` on every intake and record the concrete resolution on the
   Assignment. Resolve the verified current model-capacity posture at the same
   time. Select the recommended AI Gateway path when approved and healthy, or direct
